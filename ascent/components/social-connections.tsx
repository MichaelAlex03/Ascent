      ...
      {SOCIAL_CONNECTION_STRATEGIES.map((strategy) => {
        return (
          <Button
            ...
            <Image
              className={cn('size-4 mr-2', strategy.useTint && Platform.select({ web: 'dark:invert' }))}
              tintColor={Platform.select({
                native: strategy.useTint ? (colorScheme === 'dark' ? 'white' : 'white') : undefined,
              })}
              source={strategy.source}
            />
            <Text>Continue with {strategy.label}</Text>
          </Button>
        );
      })}
    </View>
